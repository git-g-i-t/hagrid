...
class DetectionDataset(HagridDataset):
    """
    目标检测专用数据集 (用于训练 SSD, YOLO 等)
    返回: Image, Target(BBoxes, Labels)
    """
    def __init__(self, conf: DictConfig, dataset_type: str, transform):
        super().__init__(conf, dataset_type, transform)
        # 是否开启单类模式（即不管是什么手势，标签统统设为 1）
    ...
    def __getitem__(self, index: int) -> Tuple[Image.Image, Dict]:
        ...
        if self.one_class:
            ...
            # 多类模式：处理 HaGRID 特有的 "united_bbox" 逻辑
            # 有时候一只手可能做组合动作，或者标注做了合并
            labels = []
            iter_boxes = []
        ...
        # --- 坐标转换 (归一化 -> 绝对坐标) ---
        # JSON 里存的是归一化坐标 (0.0~1.0)，Albumentations 需要绝对坐标 (Pixels)
        target = {}
        ...
        for bbox in iter_boxes:
            ...
            # 边界截断，防止坐标超出图片范围 (clip)
            if x_min < 0: x_min = 1
            if y_min > height: y_min = 1
